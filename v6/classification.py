import torch, time, tqdm, os, sys, argparse
...
trainer_config = TrainerConfig(args.data_dir, model_manager.model, args.batch_size, args.lr, args.weight_decay, model_dir=args.model_dir)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for phase in ['train', 'val']:
        ...
        for inputs, labels in tqdm.tqdm(trainer_config.dataloaders[phase], desc=f'{phase.capitalize()} Phase', unit='batch'):
        ...
        # Calculate metrics
        epoch_loss = running_loss / len(trainer_config.dataloaders[phase].dataset)
        ...
        if phase == 'val':
            if epoch_loss > best_val_loss + args.delta:
                ...
                if epoch_loss < best_val_loss:
                    ...
                    model_manager.save_checkpoint("best.pth", epoch, training_stats)
                    trainer_config.write_csv('best_conf_matrix.csv', conf_matrix)
