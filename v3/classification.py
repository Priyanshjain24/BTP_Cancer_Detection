...
class ModelTrainer:
    def __init__(self, data_dir=None, device='cpu', model_dir=None, checkpoint_path=None, batch_size=16, num_epochs=100, lr=1e-3, momentum=0.9, cancer_cells=None, model_name='RESNET18', prediction_only=False, class_weights=None, dropout_p=0.5, transform=None):
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                running_loss = 0.0
                correct_predictions = {'cancer': 0, 'non_cancer': 0}
                total = {'cancer': 0, 'non_cancer': 0}
                ...
                for inputs, labels in tqdm(self.dataloaders[phase], desc=f'{phase.capitalize()} Phase', unit='batch'):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        # Update class-wise correct/total
                        correct_predictions['cancer'] += ((labels == 1) & (preds == 1)).sum().item()
                        total['cancer'] += (labels == 1).sum().item()
                        correct_predictions['non_cancer'] += ((labels == 0) & (preds == 0)).sum().item()
                        total['non_cancer'] += (labels == 0).sum().item()
...
if __name__ == "__main__":
    trainer = ModelTrainer(D1_DATA_DIR, DEVICE, MODEL_DIR, CHK_PTH, BATCH_SIZE, NUM_EPOCHS, LR, MOMENTUM, CANCER_CELLS, MODEL, PREDICTION_ONLY, CLASS_WEIGHTS, DROPOUT)
    trainer.train()